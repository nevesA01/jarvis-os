...
Para baixar um instalador pronto, abra **Actions → Windows installer** no GitHub e baixe o artefato `Jarvis-OS-Windows` da execução concluída. A automação instala e abre o próprio instalador para validar a interface, o servidor local e os metadados de atualização antes de disponibilizá-lo. Ao publicar uma tag `v*`, a automação também anexa o instalador e o manifesto de atualização à página **Releases**. Depois da instalação inicial, o aplicativo verifica as releases estáveis do repositório e oferece baixar atualizações; não as instala sem confirmação. O instalador tem o formato `Jarvis-OS-Setup-1.0.1.exe`. A configuração de empacotamento e os arquivos do desktop ficam em `electron/`.
